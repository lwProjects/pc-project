 ...
 * @property {MessageCallback[]} callbacks - 回调函数数组
 */
interface Subscription {
  title: string;
...
class WebSocketManager {
   ...
   * @param {any} data - 消息数据
   */
  private dispatch(title: string, data: any): void {
    // 获取该 title 的所有回调函数
    ...
    console.log(`[WebSocket] 分发消息到 ${callbacks.length} 个订阅者, title: ${title}`);
   ...
   * unsubscribe();
   */
  subscribe(title: string, callback: MessageCallback): () => void {
    // 参数校验
    ...
    console.log(`[WebSocket] 订阅成功, title: ${title}, 当前订阅数: ${callbacks.length}`);

    // 返回取消订阅函数（闭包）
    return () => this.unsubscribe(title, callback);
  }
   ...
   */
  unsubscribe(title: string, callback?: MessageCallback): void {
    // 获取该 title 的回调列表
    const callbacks = this.subscriptions.get(title);
    ...
    if (callback) {
      ...
      if (index > -1) {
        ...
        console.log(`[WebSocket] 取消订阅成功, title: ${title}, 剩余订阅数: ${callbacks.length}`);
      }
